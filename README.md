# CostTrack - Track Prices & Save Money
...
3. **Add Environment Variables in Vercel**
   ...
   Go to your project settings and add all variables from `.env`:
...
4. **Update Supabase Cron Function**
   ...
   BEGIN
     PERFORM net.http_post(
       url := 'https://<your_app_name>.vercel.app/api/cron/check-prices',
       headers := jsonb_build_object(
         'Content-Type', 'application/json',
...
|   ├──actions
│      └── product.ts            # Server actions for DB operations
│   ├── auth/
│   │   └── callback/
...
├── utils/
│   └── SvgIcon.ts                      # Svg icons Utility fuctions
...
Edit the cron expression in `supabase`:
...
Customize the email template in `lib/email.ts` - modify HTML, styling, or content.
